      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500">
            Pre-loaded with perfect ChatGPT JSON response for debugging
          </p>
        </div>

        {/* Test Data Preview */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">📋 Pre-loaded Test Data</h2>
          <div className="bg-gray-50 p-4 rounded-lg">
            <h3 className="font-medium text-gray-700 mb-2">Contact Information:</h3>
            <ul className="text-sm text-gray-600 space-y-1">
              <li>• <span className="font-medium">Name:</span> {testLLMData.contact.full_name}</li>
              <li>• <span className="font-medium">Email:</span> {testLLMData.contact.email}</li>
              <li>• <span className="font-medium">Phone:</span> {testLLMData.contact.phone}</li>
              <li>• <span className="font-medium">Address:</span> {testLLMData.contact.address}</li>
              <li>• <span className="font-medium">LinkedIn:</span> {testLLMData.contact.linkedin}</li>
            </ul>
            
            <h3 className="font-medium text-gray-700 mb-2 mt-4">Work Experience ({testLLMData.work_experience.length} positions):</h3>
            <div className="space-y-3">
              {testLLMData.work_experience.map((exp, index) => {
                ...
                return (
                  <div key={index} className="border-l-2 border-blue-200 pl-3">
                    ...
                    <div className="text-xs text-gray-600">
                      {exp.start_date} - {isCurrentJob ? '(Công việc hiện tại ✓)' : exp.end_date} • {exp.location}
                    </div>
            ...
            <div className="mt-4">
              ...
              <ul className="text-sm text-gray-600 space-y-1">
                <li>• <span className="font-medium">Work Experience:</span> {testLLMData.work_experience.length} positions</li>
                <li>• <span className="font-medium">Total Bullet Points:</span> {testLLMData.work_experience.reduce((sum, exp) => sum + exp.bullets.length, 0)} bullets</li>
                <li>• <span className="font-medium">Education:</span> {testLLMData.education.length} degrees</li>
                <li>• <span className="font-medium">Skills:</span> {testLLMData.skills.length} skills</li>
                <li>• <span className="font-medium">Possibility Score:</span> {testLLMData.possibility_score}/10</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <details className="group">
            <summary className="cursor-pointer text-xl font-semibold text-gray-800 mb-4 flex items-center hover:text-blue-600 transition-colors">
...
{JSON.stringify(testLLMData, null, 2)}
        ...
        {/* Debug Info */}
        <div className="mt-8 bg-gray-900 rounded-lg p-4 text-green-400 font-mono text-xs">
          <div className="flex items-center mb-2">
            <span className="text-green-500">🔍 Debug Info:</span>
          </div>
          <div className="space-y-1">
            <div>• Source File: /Workflow Files/Initiatives/CV Parser/Manroe/2.md</div>
            <div>• No file upload required - JSON pre-loaded</div>
            <div>• No ChatGPT API calls - response cached</div>
            <div>• Focus: JSON → cvParserService → localStorage → CVEditor population</div>
            <div>• Check browser console for detailed transformation logs</div>
            <div>• Expected fix: ChatGPT "address" → API "location" → UI "Location"</div>
          </div>
        </div>
      </div>
    </div>
