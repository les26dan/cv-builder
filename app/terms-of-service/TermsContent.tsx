...
export default function TermsContent() {
  const [termsTexts, setTermsTexts] = useState<any>(null);
  ...
  useEffect(() => {
    const initLanguage = async () => {
      ...
      // Load terms texts for the detected language
      try {
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 sm:p-8 lg:p-10">
        <header className="text-center mb-8">
          ...
          <p className="text-gray-600 text-lg">
            {termsTexts.lastUpdated || (currentLanguage === 'vi' ? 'Cập nhật lần cuối: 3/8/2025' : 'Last Updated: August 3, 2025')}
          </p>
        </header>

        <div className="prose prose-lg max-w-none">
          {termsTexts.sections.map((section: any, index: number) => (
          ...
          <section className="mb-8">
            <h2 className="text-2xl font-semibold text-gray-900 mb-4">
              {currentLanguage === 'vi' ? '5. Liên Hệ' : '5. Contact'}
            </h2>
            <p className="text-gray-700 leading-relaxed mb-4">
              {currentLanguage === 'vi' 
        ...
        <div className="mt-8 pt-6 border-t border-gray-200 text-center">
          <p className="text-sm text-gray-500">
            {currentLanguage === 'vi'
              ? 'Các điều khoản này có thể được cập nhật theo thời gian. Phiên bản mới nhất sẽ luôn có sẵn trên trang web này.'
              : 'These terms may be updated from time to time. The latest version will always be available on this website.'
            }
          </p>
        </div>
      </div>
