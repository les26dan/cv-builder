...
export class PDFGenerationService {
  ...
  public async generatePDF(
    ...
    try {
      console.log('🚀 PDFGenerationService: Starting PDF generation process...');
      console.log('📊 PDFGenerationService: CV Data analysis:', {
        hasId: !!cvData.id,
      ...
      console.log('🔧 PDFGenerationService: Initializing libraries...');
      await this.initializeLibraries();

      // Generate cache key for deduplication
      const dataHash = this.generateDataHash(cvData);
      const cacheKey = `${cvData.id || 'unknown'}-${dataHash}`;
      console.log('🔑 PDFGenerationService: Cache key generated:', cacheKey.substring(0, 20) + '...');

      // Check cache first (if enabled)
      if (options.useCache !== false) {
        const cached = this.getCachedPDF(cacheKey);
        if (cached) {
          console.log('📋 PDFGenerationService: Using cached PDF');
          return {
            success: true,
            pdfBlob: cached.blob,
      ...
      console.log('🔄 PDFGenerationService: Generating new PDF...');
      console.log('📚 PDFGenerationService: Available libraries:', {
        jsPDF: !!jsPDF,
        pdfLib: !!pdfLib,
        pdfjsLib: !!pdfjsLib
      });
  ...
  private async generateWithJsPDF(
    ...
    try {
      ...
      // 2. SUMMARY SECTION - DEBUG THE ACTUAL DATA STRUCTURE
      console.log('📄 jsPDF: FULL CV DATA STRUCTURE:', JSON.stringify(cvData, null, 2));
      console.log('📄 jsPDF: Summary field type:', typeof cvData.summary);
      console.log('📄 jsPDF: Summary field value:', cvData.summary);
      
      const summaryContent = cvData.summary?.content;
      console.log('📄 jsPDF: Summary content access:', {
        hasSummary: !!cvData.summary,
        hasContent: !!summaryContent,
        contentLength: summaryContent?.length || 0,
        contentPreview: summaryContent?.substring(0, 50) + '...'
      });
      ...
      if (summaryContent && summaryContent.trim()) {
        ...
        console.log('📄 jsPDF: No summary content found');
      }
      ...
      if (cvData.skills?.items?.length > 0) {
        ...
        console.log('📄 jsPDF: Adding skills section:', cvData.skills.items.length, 'skills');
