...
Phụ lục này mô tả cấu hình và nội dung tổng quát của các prompt dùng cho phương pháp LLM Reasoning trong Chương 5 và cho module phân tích CV trong hệ thống. Nội dung đầy đủ của prompt cùng schema JSON được công bố trong mã nguồn của khóa luận.

\section{Cấu hình LLM dùng cho thí nghiệm Chương 5}

Bảng~\ref{tab:llm-config-ch5} liệt kê các tham số gọi mô hình \texttt{gpt-4o-mini} của OpenAI thông qua giao thức Chat Completions trong module đánh giá.

\begin{table}[H]
\centering
\caption{Tham số gọi LLM cho thí nghiệm matching CV–JD}
...
Bảng~\ref{tab:llm-schema} mô tả các trường trong JSON đầu ra của LLM. Module \texttt{llmMatcher.ts} parse JSON, ép kiểu và giới hạn miền giá trị cho từng trường trước khi ghi vào tệp kết quả.

\begin{table}[H]
\centering
\caption{Schema JSON đầu ra của LLM matcher (Chương 5)}
...
Module \texttt{jd-analyze} của hệ thống ngoài việc chấm điểm còn sinh gợi ý cải thiện cho từng phần của CV nên có prompt và schema phong phú hơn so với §C.1. Bảng~\ref{tab:llm-compare} liệt kê các điểm khác biệt chính.

\begin{table}[H]
\centering
\caption{So sánh cấu hình LLM giữa thí nghiệm Chương 5 và module sản phẩm}
\label{tab:llm-compare}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Tham số} & \textbf{Thí nghiệm Ch.5} & \textbf{Module jd-analyze} \\
\hline
...
Vì payload đầu ra dài (\texttt{max\_tokens = 3000}), prompt của module sản phẩm không phù hợp cho thí nghiệm Chương 5: nếu dùng cho 400 cặp đánh giá, chi phí ước tính sẽ tăng khoảng 10 lần so với cấu hình §C.1, vượt ngưỡng ngân sách đặt trong khóa luận.

\section{Các prompt khác trong hệ thống}

Bảng~\ref{tab:other-llm} liệt kê các điểm API còn lại có sử dụng LLM nhưng không tham gia vào thí nghiệm Chương 5.
