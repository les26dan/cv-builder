...
export class GoogleOAuthProvider implements IOAuthProvider {
  ...
  public async fetchUserProfile(accessToken: string): Promise<OAuthUserProfile> {
    ...
    try {
      ...
      // Google profile received successfully

      // Validate required fields (Google userinfo API uses 'id' instead of 'sub')
      const userId = googleProfile.sub || googleProfile.id;
      if (!googleProfile.email || !userId) {
        console.error('❌ Missing required fields in Google profile:', {
          email: googleProfile.email,
          sub: googleProfile.sub,
          id: googleProfile.id,
          allFields: Object.keys(googleProfile)
        });
